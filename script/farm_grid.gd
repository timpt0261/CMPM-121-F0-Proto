...
func get_adjacent_cells(cell_position: Vector2i) -> Array:
	...
	for j in range(-1, 1):
		for i in range(-1, 1):
			...
			if adjacent_pos == cell_position || adjacent_index < 0 || adjacent_index >= width*height:
				continue
...
func decode_cell(position: Vector2i) -> Cell:
	...
	return Cell.new(position, hydration, sunlight, plant_type_id, plant_growth, plant_turns_alive, plant_template_list[plant_type_id])
