...
func update_plant_sprite(cell: Cell):
	...
	atlas_tex.atlas = load(terrain_map.farm_grid.plant_template_list[cell.plant_type_id].get("sprite"));
	atlas_tex.region = Rect2(cell.plant_visual_phase * PLANT_SPRITES_SIZE, 0, PLANT_SPRITES_SIZE, PLANT_SPRITES_SIZE);
...
func pixel_in_bounds(pixel_pos: Vector2i) -> bool:
	...
	return relative.x >= 0 && relative.x < pixel_width && relative.y >= 0 && relative.y < pixel_height
