...
enum DataIndices { CURRENT_SNAPSHOT = 0, SNAPSHOT_SIZE = 1, SNAPSHOTS_START = 9 }
...
func get_snapshots() -> Array[GameStateArray]:
	...
	while snapshot_index <= loop_bound:
		snapshots.append(
			GameStateArray.from_byte_array(
				byte_array.slice(snapshot_index, snapshot_index + snapshot_size)
			)
		)
